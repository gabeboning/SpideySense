...
int movementThreshold = 300, blurRadius = 5;

float imageThreshold = .7;
...
void setup() {
  ...
  flob.setOm(0).setMinNumPixels(150).setMaxNumPixels(3000);
...
void findBlobs() {
  ...
  image(buffer,0,0); // display it so we can threshold
  //filter(THRESHOLD, imageThreshold); // turn black and white again

  blobs = flob.tracksimple(buffer.get(0,0,buffer.width, buffer.height)); // get the blobs
